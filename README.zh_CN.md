...
* **带宽优化下行**：将 `enable_pure_downlink` 设为 `false` 后，下行会把传输字节流拆成 6bit 片段，复用原有的填充池与 ASCII/低熵/customised 偏好，降低下行开销；上行保持sudoku本身协议，下行特征此时与上行保持一致。此模式不要求开启 AEAD，但两端配置必须一致。
