...
* **Bandwidth-Optimized Downlink**: After setting `enable_pure_downlink` to `false`, the downlink splits the byte stream into 6-bit chunks and reuses the existing padding pool together with the ASCII/low-entropy/customized preferences to reduce downlink overhead. The uplink still uses the native Sudoku protocol, and the downlink fingerprint remains aligned with the uplink.
